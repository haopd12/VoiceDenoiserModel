...
class AudioDataset(Dataset):
    def __init__(self, files: List[str], sr: int, noise=None) -> None:
        super().__init__()
        self.files = []
        for file in files:
    ...
    def __getitem__(self, index) -> Tuple[str, Tensor, int]:
        ...
        if self.noise is not None:
            _, _, audio = mix_at_snr(audio, self.noise, self.sr)
...
def main(args):
    ...
    noisy_fn = args.noisy_fn   
    if noisy_fn is not None: 
        noise, _ = load_audio(NOISES[noisy_fn], df_sr, "cpu")
        
    else:
        noise = None

    ds = AudioDataset(input_files, df_sr, noise)
